class Deals(object):
    def __init__(self, client):
        self.client = client

    def create_deal(self, data):
        """
        Create a new deal


        Args:
            data:

        Returns:

        """
        return self.client._post("/deals", json=data)

    def retrieve_deal(self, deal_id):
        """
        Retrieve an existing deal


        Args:
            deal_id:

        Returns:

        """
        return self.client._get("/deals/{}".format(deal_id))

    def update_deal(self, deal_id, data):
        """
        Update an existing deal


        Args:
            deal_id:
            data:

        Returns:

        """
        return self.client._put("/deals/{}".format(deal_id), json=data)

    def delete_deal(self, deal_id):
        """
        Delete an existing deal


        Args:
            deal_id:

        Returns:

        """
        return self.client._delete("/deals/{}".format(deal_id))

    def list_all_deals(self, **params):
        """
        Retrieve all existing deals


        Returns:

        """
        return self.client._get("/deals", params=params)

    def create_deal_note(self, deal_id, data):
        """
        Create a new note for a deal


        Args:
            deal_id:
            data:

        Returns:

        """
        return self.client._post("/deals/{}/notes".format(deal_id), json=data)

    def update_deal_note(self, deal_id, note_id, data):
        """
        Update an existing note for a deal


        Args:
            deal_id:
            note_id:
            data:

        Returns:

        """
        return self.client._put("/deals/{}/notes/{}".format(deal_id, note_id), json=data)

    def create_pipeline(self):
        raise NotImplementedError

    def retrieve_pipeline(self):
        raise NotImplementedError

    def update_pipeline(self):
        raise NotImplementedError

    def delete_pipeline(self):
        raise NotImplementedError

    def list_all_pipelines(self, **params):
        """
        Retrieve all existing pipelines


        Returns:

        """
        return self.client._get("/dealGroups", params=params)

    def create_stage(self):
        raise NotImplementedError

    def retrieve_stage(self):
        raise NotImplementedError

    def update_stage(self):
        raise NotImplementedError

    def delete_stage(self):
        raise NotImplementedError

    def list_all_stages(self, **params):
        """
        Retrieve all existing stages


        Returns:

        """
        return self.client._get("/dealStages", params=params)

    def move_deals_to_another_stage(self):
        raise NotImplementedError

    def create_custom_field(self):
        raise NotImplementedError

    def retrieve_custom_fields_by_id(self, field_id):
        """
        Retrieve all existing custom fields


        Args:
            field_id:

        Returns:

        """
        return self.client._get("/deals/{}/dealCustomFieldData".format(field_id))

    def update_custom_field(self):
        raise NotImplementedError

    def delete_custom_field(self):
        raise NotImplementedError

    def list_all_custom_fields(self, **params):
        """

        Returns:

        """
        return self.client._get("/dealCustomFieldMeta", params=params)

    def create_custom_field_value(self):
        raise NotImplementedError

    def bulk_create_custom_field_value(self):
        raise NotImplementedError

    def retrieve_custom_field_value(self):
        raise NotImplementedError

    def update_custom_field_value(self):
        raise NotImplementedError

    def bulk_update_custom_field_value(self):
        raise NotImplementedError

    def delete_custom_field_value(self):
        raise NotImplementedError

    def list_all_custom_field_values(self, **params):
        """

        Returns:

        """
        return self.client._get("/dealCustomFieldData", params=params)

    def create_secondary_contact(self):
        raise NotImplementedError

    def retrieve_secondary_contact(self):
        raise NotImplementedError

    def update_secondary_contact(self):
        raise NotImplementedError

    def delete_secondary_contact(self):
        raise NotImplementedError

    def list_all_secondary_contacts(self):
        raise NotImplementedError
